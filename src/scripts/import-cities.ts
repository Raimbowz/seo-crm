...
async function bootstrap() {
  ...
  try {
    ...
    if (!filePath) {
      throw new Error('Could not find the cities_inflection.csv file. Please check its location.');
    }
    ...
    for (let i = 0; i < cities.length; i += batchSize) {
      ...
      for (const cityData of batch) {
        try {
          ...
          console.error(`Failed to import city "${cityData.name}":`, error.message);
          failureCount++;
        }
      }
    }
